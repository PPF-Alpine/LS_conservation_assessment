...
luc_2100_ssp1_26_selected <- rast(paste0(data_storage_path, "Datasets/human_footprint/LUH_2/luc_2100_ssp1_26_selected.tif"))
...
extracted_values <- terra::extract(luc_2100_ssp1_26_selected, alpine_shapes_vect, ID = TRUE)
...
luc_summary_ssp1_26 <- luc_summary |>
  filter(!layer %in% c("area_sz", "log_are"))|>
  group_by(Mntn_rn)|>
...
# Define file path
output_path <- paste0(data_storage_path, "Outputs/land_use_change/all_mountains_luc_summary_ssp1_26_", today, ".csv")

write.csv(luc_summary_ssp1_26,output_path)
