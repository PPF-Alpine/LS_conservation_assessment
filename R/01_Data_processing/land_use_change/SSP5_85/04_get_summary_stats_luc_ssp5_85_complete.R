...
polygons<-alpine_shapes$Mntn_rn


luc_2100_ssp5_85_selected <- rast(paste0(data_storage_path, "Datasets/human_footprint/LUH_2/luc_2100_ssp5_85_selected.tif"))
...
extracted_values <- terra::extract(luc_2100_ssp5_85_selected, alpine_shapes_vect, ID = TRUE)
...
luc_summary_ssp5_85 <- luc_summary |>
  filter(!layer %in% c("area_sz", "log_are"))|>
  group_by(Mntn_rn)|>
  mutate(sum_luc =sum(mean_luc))|>
...
# Define file path
output_path <- paste0(data_storage_path, "Outputs/land_use_change/all_mountains_luc_summary_ssp5_85_", today, ".csv")

write.csv(luc_summary_ssp5_85,output_path)
