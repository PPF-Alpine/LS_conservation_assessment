...
annual_temp_ssp26 <- rast(paste0(data_storage_path,"Datasets/Mountains/Chelsa/CHELSA_bio1_2071-2100_mpi-esm1-2-hr_ssp126_V.2.1.tif"))

annual_prec_ssp26 <- rast(paste0(data_storage_path,"Datasets/Mountains/Chelsa/CHELSA_bio12_2071-2100_mpi-esm1-2-hr_ssp126_V.2.1.tif"))
...
for (mnt in mountain_selection) {
  ...
  temp_mountain_future <- mask(crop(annual_temp_ssp26, mountain_range), mountain_range)
  prec_mountain_future <- mask(crop(annual_prec_ssp26, mountain_range), mountain_range)
...
for (i in seq_along(plot_chunks)) {
  ...
  # Save
  ggsave(
    filename = file.path(output_folder, paste0("climate_space_shift_ssp126_page_", i, "_", today, ".jpg")),
    plot = p,
    width = 14, height = 10,
    dpi = 300, device = "jpeg"
  )
...
# Define file path
output_path <- paste0(data_storage_path, "Outputs/mountain_climates/all_mountains_climate_shifts_ssp126_2100_", today, ".csv")

write.csv(climate_shifts,output_path)

# Define file path
output_path <- paste0(data_storage_path, "Outputs/mountain_climates/climate_combined_all_ssp126_2100_", today, ".csv")
