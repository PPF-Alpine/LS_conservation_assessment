...
annual_prec_future <- rast(paste0(data_storage_path,"Datasets/Mountains/Chelsa/time_slices_2055/prec_2055_585.tif"))
...
output_dir <- paste0(data_storage_path, "Outputs/mountain_climates/future_alpine_area_timestamps/")
...
for (mountain_name in mountain_selection) {
  ...
  temp_mountain_future <- mask(crop(annual_temp_future, mountain_range), mountain_range)
  prec_mountain_future <- mask(crop(annual_prec_future, mountain_range), mountain_range)
  ...
  p <- ggplot(future_alpine_df, aes(x = x, y = y, fill = cells)) +
    ...
    coord_equal() +
    theme_minimal() +
    labs(title = paste0(mountain_name, ": future alpine climate space (RCP8.5 2055)"),
         fill = "Climate space",
  ...
  # Save plot
  ggsave(filename = paste0(output_dir, "/", gsub(" ", "_", mountain_name), "_climate_loss_map_2055_585.jpeg"),
...
# Define file path
output_path <- paste0(data_storage_path, "Outputs/mountain_climates/future_alpine_area/all_mountains_perc_loss_585_55_", today, ".csv")
