...
total_endemism_join_test <- total_endemism_join |>
  left_join(
    species_list |>
      select(species, status_summary_global) |>
      distinct(),
    by = "species"
  )

# nationally threathened
total_endemism_join_national <- total_endemism_join |>
  left_join(
    species_list |>
      select(species, status_summary_national) |>
      distinct(),
    by = "species"
  )
...
dd_and_NA <- total_endemism_join_test |>
  filter(
    status_summary_global == "data deficient" |
      status_summary_global == "NA")

threatened_national <- total_endemism_join_national|>
...
target_keys <- str_replace_all(dd_and_NA$species, " ", "_")
...
target_species <- threatened_national$species
...
out_file <- file.path(data_storage_path, "Datasets", "species_list","species_richness","biodiv_dimensions_0918","globally_dd_species.tif")
