...
border_segments <- sf::st_read(paste0(data_storage_path, "Datasets/transboundary/GADM/GADM_pairwiseborders_new.gpkg"))
...
total_endemism_join <- read.csv(paste0(data_storage_path,"Datasets/species_list/species_endemism/total_species_endemism.csv"))  |>
  select(-average_min_elevation,-average_max_elevation,-elev_range)|>
  filter(pct_in_HKH_area >0 )
...
total_endemism_join_test <- total_endemism_join |>
  left_join(
    species_list |>
      select(species, status_summary_global,status_summary_national,average_min_elevation,average_max_elevation) |>
      distinct(),
    by = "species"
  )


threatened <- total_endemism_join_test|> filter(status_summary_global=="threatened" | status_summary_global=="data deficient")
#threatened <- total_endemism_join_test|> filter(status_summary_global=="threatened")

length(unique(threatened$species))
