...
#----------------------------------------------------------#
pa <- st_transform(pa, crs(prop_threathened))   # make sure both have the same projection
total_richness <- global(prop_threathened, "sum", na.rm = TRUE)[[1]]

richness_in_pa <- mask(prop_threathened, vect(pa))
...
vals <- values(prop_threathened, na.rm = TRUE)
...
priority_cells <- prop_threathened >= thresh
plot(priority_cells)
...
sensitivity_priority <- function(prop_threathened, pa,
                                 probs = seq(0.5, 0.95, by = 0.05),
                                 tie_rule = c(">", ">="),
                                 plot = TRUE,
                                 area_km2 = TRUE) {
  stopifnot(inherits(prop_threathened, "SpatRaster"), nlyr(prop_threathened) == 1)
  ...
  if (area_km2) {
    ...
    cell_area <- cellSize(prop_threathened, unit = "km")
  }
  ...
  results <- lapply(probs, function(p) {
    ...
    priority_cells <- if (tie_rule == ">") ifel(prop_threathened >  thr, 1, NA)
    else ifel(prop_threathened >= thr, 1, NA)
    
    total_priority_cells <- global(priority_cells, "sum", na.rm = TRUE)[[1]]
    
    # Richness restricted to priority cells
    priority_richness <- mask(prop_threathened, priority_cells)
...
plot<- ggplot(res_mod_threat, aes(x = top_share, y = prop_rich_in)) +
  ...
  labs(
    ...
    title = "How well protected are areas with the highest proportions of threathened mammals?"
  ) +
...
ggsave(
  filename = paste0(data_storage_path, "Datasets/species_list/species_richness/threathened_richness_in_PA.png"),
  plot = plot,
  width = 12,
  height = 6,
  dpi = 300
